...
public class DoubleSumCInequalityTheory extends DoubleInequalityTheory  implements SumCTheory{
	//maxSumC : the biggest sum sumConstant
	...
	private List<DataValue<Double>> makeNewPotsWithSumC(List<DataValue<Double>> dvs) {
		Set<DataValue<Double>> dvSet = new HashSet<>(dvs);
		Stream<DataValue<Double>> valAndSums = dvs.stream()
		...
		List<DataValue<Double>> valAndSumsAndConsts = valAndSums.collect(Collectors.toList()); 
	...
	public Determinizer<Double> getDeterminizer() {
		return new SumCInequalityDeterminizer<Double>(this, sortedSumConsts);
	}
